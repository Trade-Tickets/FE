...
export function EventExplorer({ onEventClick }: EventExplorerProps) {
  ...
  useEffect(() => {
    fetchEvents(page, 6).then(setEvents).catch(() => setEvents([]));
  }, [page]);
  ...
  const filteredEvents = useMemo(() => {
    return events.filter((event) => {
      ...
      const matchesCategory =
        ...
        event.tags.some((tag) =>
          tag.toUpperCase().includes(selectedCategory.toUpperCase()),
        );
      ...
      {filteredEvents.length === 0 ? (
        ...
        <>
          ...
          <div className="flex items-center justify-center gap-3 mt-8">
            <Pagination 
              currentPage={page} 
              totalPages={events.length === 6 ? page + 2 : page + 1} 
...
function EventCard({
  ...
  return (
    ...
    >
        ...
        <div
          className={`w-1/2 p-4 flex flex-col justify-center ${topStat && topStat.change24h >= 0 ? "bg-brand-green/20" : "bg-[#ff5f56]/20"}`}
        >
          <p className="font-extrabold text-[10px] text-gray-500 tracking-widest uppercase mb-1">
            24H Change
          </p>
          <div className="flex items-center gap-1">
            {topStat ? (
              <>
                ...
                <p
                  className={`font-black text-lg font-mono ${topStat.change24h >= 0 ? "text-green-700" : "text-red-700"}`}
                >
                  {topStat.change24h > 0 ? "+" : ""}
                  {topStat.change24h}%
                </p>
